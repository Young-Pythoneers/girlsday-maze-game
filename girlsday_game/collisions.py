...
class Collisions:
    ...
    def collision(selfs, ent1, ent2):
        penetration = 0
        distance = math.sqrt(
            math.pow(ent1.rect.centerx - ent2.rect.centerx, 2)
            + math.pow(ent1.rect.centery - ent2.rect.centery, 2)
        )
        if distance > 50:
            return False
    ...
    def apply_collisions(self, entities):
        ...
        for ent in entities:
            if isinstance(ent, PhysicalEntity):
                if (
                    ent.rect.left < 0
                    or ent.rect.right > self.game.display.screen_size_x
                ):
                    ent.impulse_x = -ent.impulse_x
                if (
                    ent.rect.top < 0
                    or ent.rect.bottom > self.game.display.screen_size_y
                ):
                    ent.impulse_y = -ent.impulse_y
                ent.rect.left = np.clip(
                    ent.rect.left, 0, self.game.display.screen_size_x - ent.rect.width
                )
                ent.rect.top = np.clip(
                    ent.rect.top, 0, self.game.display.screen_size_y - ent.rect.height
                )
